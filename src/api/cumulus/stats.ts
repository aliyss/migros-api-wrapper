...
const url = migrosApiPaths["login"] + "/cumulus/dashboard/stats"

async function getCumulusStatsRequest(url: string, cookies: ILoginCookies): Promise<Record<string, any>> {
...
export async function getCumulusStats(cookies: ILoginCookies): Promise<any> {
	return getCumulusStatsRequest(url, cookies)
}